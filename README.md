...
```text
   用户   chenpi /home/chenpi
   仓库   ~/chenpi_file/chenpi_dotfiles
   模块   4 个可选 + 6 个必装
   1) 开始安装
   2) 查看系统信息
   3) 选择可选模块
...
选 2 会把系统情况全列一遍（不动任何东西），选 3 用 fzf 勾可选模块，选 1 才真正开始装。

**屏幕上只会看到正在执行的命令和简短结果**，解说性的内容全写进日志（`/tmp/chenpi-install.log`，跑完复制到 `~/Documents/`）。想连解说一起看：`CHENPI_VERBOSE=1 ./install.sh`。
